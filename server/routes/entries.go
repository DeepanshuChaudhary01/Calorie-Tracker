...
func DeleteEntry(c *gin.Context) {
	entryID := c.Params.ByName("id")
	//above id is in json format
	docID, _ := primitive.ObjectIDFromHex(entryID)
	//converting it to bson format

	var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
	//This means that any operations performed within this context will be canceled if they take longer than the specified timeout
